...
export default function Home() {
  ...
  const [villaCouponCode, setVillaCouponCode] = useState("");
  const [vehicleCouponCode, setVehicleCouponCode] = useState("");
  const [villaCoupon, setVillaCoupon] = useState<{id: number; name: string; discountType: string; discountValue: number} | null>(null);
  const [vehicleCoupon, setVehicleCoupon] = useState<{id: number; name: string; discountType: string; discountValue: number} | null>(null);
  ...
  const applyCoupon = async (category: "villa" | "vehicle") => {
    ...
    if (!code.trim()) return;
    setCouponError(prev => ({ ...prev, [category]: undefined }));
    setCouponLoading(prev => ({ ...prev, [category]: true }));
    try {
      const res = await fetch("/api/validate-coupon", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: code.trim(), category }) });
      const data = await res.json();
      if (!res.ok) { setCouponError(prev => ({ ...prev, [category]: data.error })); return; }
      if (category === "villa") setVillaCoupon(data);
      else setVehicleCoupon(data);
    } catch { setCouponError(prev => ({ ...prev, [category]: "쿠폰 확인 실패" })); }
    finally { setCouponLoading(prev => ({ ...prev, [category]: false })); }
  };

  const calcDiscount = (total: number, coupon: {discountType: string; discountValue: number} | null) => {
    if (!coupon) return 0;
    if (coupon.discountType === "percent") return Math.round(total * coupon.discountValue / 100);
                ...
                <SectionCard title={t("villa.title")} icon={Plane} isEnabled={field.value ?? false} onToggle={field.onChange} gradient="from-blue-500/10">
                  ...
                  {(villaEstimate.price > 0 || (loadedQuoteId && breakdown?.villa?.price)) && (
                    <div className="mt-4 bg-gradient-to-r from-blue-600 to-blue-500 text-white p-4 rounded-xl shadow-lg">
                      ...
                      {villaCoupon && (() => {
                        const villaTotal = loadedQuoteId && breakdown?.villa?.price ? breakdown.villa.price : villaEstimate.price;
                        const disc = calcDiscount(villaTotal, villaCoupon);
                        return disc > 0 ? (
                          <div className="mt-2 pt-2 border-t border-blue-400/30 space-y-1">
                            <div className="flex justify-between text-xs text-green-200"><span>🎟 {villaCoupon.name} ({villaCoupon.discountType === "percent" ? `${villaCoupon.discountValue}%` : `$${villaCoupon.discountValue}`})</span><span>-${disc}</span></div>
                            <div className="flex justify-between font-bold text-sm"><span>할인 적용 금액</span><span>${villaTotal - disc}</span></div>
                          </div>
                        ) : null;
                      })()}
                ...
                >
                      ...
                      {values.vehicle?.selections?.map((selection, index) => (
                        <div key={`vehicle-day-${index}`} className="grid grid-cols-1 md:grid-cols-7 gap-3 p-4 bg-white rounded-xl border border-slate-200 relative group shadow-sm items-end">
                                    ...
                                    return (
                                      <div className="space-y-0.5">
                                        {descText.split(/[|\n]/).map((line: string, i: number) => (
                                          line.trim() ? <p key={i} className={i === 0 ? "font-bold text-slate-700" : ""}>{line}</p> : <div key={i} className="h-2" />
                                        ))}
                                      </div>
                                    );
                                  })()}
                          ...
                          <div className="md:col-span-2 space-y-1.5"><Label className="text-xs font-semibold text-slate-500">{t("vehicle.route")}</Label><Controller control={form.control} name={`vehicle.selections.${index}.route`} render={({ field }) => {
                            ...
                            return (<Select onValueChange={(value) => { field.onChange(value); if (document.activeElement instanceof HTMLElement) { document.activeElement.blur(); } }} value={field.value}><SelectTrigger className="h-10 rounded-lg text-sm bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 dark:text-slate-100"><SelectValue placeholder={t("vehicle.select")} /></SelectTrigger><SelectContent className="z-[9999] bg-white dark:bg-slate-800 border shadow-lg opacity-100 dark:border-slate-600">{selVt?.cityLabel && <SelectItem value="city">{selVt.cityLabel}</SelectItem>}{selVt?.onewayLabel && <SelectItem value="oneway">{selVt.onewayLabel}</SelectItem>}{selVt?.hochamOnewayLabel && <SelectItem value="hocham_oneway">{selVt.hochamOnewayLabel}</SelectItem>}{selVt?.phanthietOnewayLabel && <SelectItem value="phanthiet_oneway">{selVt.phanthietOnewayLabel}</SelectItem>}{selVt?.roundtripLabel && <SelectItem value="roundtrip">{selVt.roundtripLabel}</SelectItem>}{selVt?.cityPickupDropLabel && <SelectItem value="city_pickup_drop">{selVt.cityPickupDropLabel}</SelectItem>}{selVt?.customRoutes?.map((cr) => (<SelectItem key={cr.key} value={`custom_${cr.key}`}>{cr.label}</SelectItem>))}</SelectContent></Select>);
                          }} /></div>
                  ...
                  {(loadedQuoteId && breakdown?.vehicle?.price) || (values.vehicle?.selections && values.vehicle.selections.length > 0) ? (
                    <div className="mt-4 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white p-4 rounded-xl shadow-lg">
                            ...
                            ${loadedQuoteId && breakdown?.vehicle?.price 
                              ...
                              : (() => {
                                  const getVtPrice = (type: string, route: string) => {
                                    const vt = vehicleTypesData.find(v => v.key === type);
                                    if (!vt) return 0;
                                    const m: Record<string, number> = { city: vt.cityPrice, oneway: vt.onewayPrice, hocham_oneway: vt.hochamOnewayPrice, phanthiet_oneway: vt.phanthietOnewayPrice, roundtrip: vt.roundtripPrice, city_pickup_drop: vt.cityPickupDropPrice, ...(vt.customRoutes || []).reduce((acc, cr) => ({ ...acc, [`custom_${cr.key}`]: cr.price }), {} as Record<string, number>) };
                                    return m[route] || 0;
                                  };
                      ...
                      <div className="text-xs text-indigo-100 space-y-1">
                        {loadedQuoteId && breakdown?.vehicle?.description ? (
                          ...
                          values.vehicle?.selections?.map((sel, i) => {
                            ...
                            const vt = vehicleTypesData.find(v => v.key === sel.type);
                            const m: Record<string, number> = vt ? { city: vt.cityPrice, oneway: vt.onewayPrice, hocham_oneway: vt.hochamOnewayPrice, phanthiet_oneway: vt.phanthietOnewayPrice, roundtrip: vt.roundtripPrice, city_pickup_drop: vt.cityPickupDropPrice, ...(vt.customRoutes || []).reduce((acc, cr) => ({ ...acc, [`custom_${cr.key}`]: cr.price }), {} as Record<string, number>) } : {};
                            const routeLabels: Record<string, string> = vt ? { city: vt.cityLabel || t("route.city"), oneway: vt.onewayLabel || t("route.oneway"), hocham_oneway: vt.hochamOnewayLabel || t("route.hocham_oneway"), phanthiet_oneway: vt.phanthietOnewayLabel || t("route.phanthiet_oneway"), roundtrip: vt.roundtripLabel || t("route.roundtrip"), city_pickup_drop: vt.cityPickupDropLabel || t("route.city_pickup_drop"), ...(vt.customRoutes || []).reduce((acc, cr) => ({ ...acc, [`custom_${cr.key}`]: cr.label }), {} as Record<string, string>) } : {};
                      ...
                      {vehicleCoupon && (() => {
                        const vTotal = loadedQuoteId && breakdown?.vehicle?.price ? breakdown.vehicle.price : (values.vehicle?.selections?.reduce((sum, sel) => { if (!sel?.type || !sel?.route) return sum; const vt = vehicleTypesData.find(v => v.key === sel.type); if (!vt) return sum; const m: Record<string, number> = { city: vt.cityPrice, oneway: vt.onewayPrice, hocham_oneway: vt.hochamOnewayPrice, phanthiet_oneway: vt.phanthietOnewayPrice, roundtrip: vt.roundtripPrice, city_pickup_drop: vt.cityPickupDropPrice, ...(vt.customRoutes || []).reduce((acc, cr) => ({ ...acc, [`custom_${cr.key}`]: cr.price }), {} as Record<string, number>) }; return sum + (m[sel.route] || 0); }, 0) || 0);
                        const disc = calcDiscount(vTotal, vehicleCoupon);
                        return disc > 0 ? (
                          <div className="mt-2 pt-2 border-t border-indigo-400/30 space-y-1">
                            <div className="flex justify-between text-xs text-green-200"><span>🎟 {vehicleCoupon.name} ({vehicleCoupon.discountType === "percent" ? `${vehicleCoupon.discountValue}%` : `$${vehicleCoupon.discountValue}`})</span><span>-${disc}</span></div>
                            <div className="flex justify-between font-bold text-sm"><span>할인 적용 금액</span><span>${vTotal - disc}</span></div>
                          </div>
                        ) : null;
                      })()}

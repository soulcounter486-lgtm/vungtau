                                    ...
                                    return (
                                      <div key={pi} className="mb-1">
                                        ...
                                        <div className="flex gap-1 items-center">
                                          ...
                                          {(() => { const rawUnavail = quote.ecoUnavailableProfiles; const unavail: number[] = Array.isArray(rawUnavail) ? rawUnavail.map(Number) : []; const pickedIds = [person.first, person.second, person.third].filter((v): v is number => v !== null && v !== undefined && v !== 0).map(Number); const allUnavailable = pickedIds.length > 0 && pickedIds.every(id => unavail.includes(id)); return (<span className={`text-[8px] font-bold ml-1 leading-tight text-center whitespace-pre-line px-1 py-0.5 rounded ${allUnavailable ? "text-red-400 bg-red-900/30" : "text-yellow-400 bg-yellow-900/30"}`}>{allUnavailable ? (language === "ko" ? "다른\n에코픽\n부탁드립니다" : "Please\npick\nagain") : `p:${pickedIds.join(",")} u:${unavail.join(",")}`}</span>); })()}
                                        </div>
                                      </div>
                                    );
                                  })}

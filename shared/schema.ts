...
export const quotes = pgTable("quotes", {
  ...
  ecoUnavailableProfiles: jsonb("eco_unavailable_profiles").$type<number[]>().default([]), // 픽불가 프로필 ID 배열
...
export const coupons = pgTable("coupons", {
  id: serial("id").primaryKey(),
  code: text("code"), // 쿠폰 코드 (사용자가 입력)
  name: text("name").notNull(), // 쿠폰 이름
  description: text("description"), // 쿠폰 설명
  category: text("category").default("all"), // villa, vehicle, all
  ...
  maxUses: integer("max_uses"), // 최대 사용 횟수 (null=무제한)
  currentUses: integer("current_uses").default(0), // 현재 사용 횟수
  isActive: boolean("is_active").default(true), // 활성화 여부
...
export const vehicleTypes = pgTable("vehicle_types", {
  ...
  cityLabel: text("city_label").default("붕따우 시내"),
  onewayPrice: integer("oneway_price").notNull().default(0),
  onewayLabel: text("oneway_label").default("호치민 ↔ 붕따우 (편도)"),
  hochamOnewayPrice: integer("hocham_oneway_price").notNull().default(0),
  hochamOnewayLabel: text("hocham_oneway_label").default("호치민 ↔ 호짬 (편도)"),
  phanthietOnewayPrice: integer("phanthiet_oneway_price").notNull().default(0),
  phanthietOnewayLabel: text("phanthiet_oneway_label").default("호치민 ↔ 판티엣 (편도)"),
  roundtripPrice: integer("roundtrip_price").notNull().default(0),
  roundtripLabel: text("roundtrip_label").default("호치민 ↔ 붕따우 (왕복)"),
